...
execute at @e[type=item,nbt={Item:{id:"minecraft:apple"}}] if block ~ ~-.5 ~ farmland run execute as @e[type=item,nbt={Item:{id:"minecraft:bone_meal",Count:3b}},limit=1,sort=nearest,distance=0..1] at @s run summon minecraft:armor_stand ~ ~ ~ {Tags:["appleTree"]}
